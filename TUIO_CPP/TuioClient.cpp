	...
	try {
		...
		if( strcmp( msg.AddressPattern(), "/tuio/2Dobj" ) == 0 ){
			...
			if( strcmp( cmd, "set" ) == 0 ){	
				...
				} else if ( ((*tobj)->getX()!=xpos) || ((*tobj)->getY()!=ypos) || ((*tobj)->getAngle()!=angle) ) {
							...
							if (deleteCursor->getFingerID()==maxFingerID) {
								...
								if (cursorList.size()>0) {
									...
									for (flist=freeCursorList.begin(); flist != freeCursorList.end(); flist++) {
										if ((*flist)->getFingerID()>=maxFingerID) { 
											freeCursorList.erase(flist);
											flist--;
										}
									}
								} 
							} else freeCursorList.push_back(deleteCursor);
